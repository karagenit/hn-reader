...
- It's a git checkout with `origin` = `https://github.com/karagenit/hn-reader`, tracking `main` as of 2026-08-16 (previously tracked `debug` — check `sudo -u hn-reader git -C /var/www/hn-reader status` to confirm which branch is actually checked out before pulling, since this may change over time).
